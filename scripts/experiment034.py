...
def quantized(inp):
    N_BINS = 5
    out = np.zeros(shape=(N_SEQ_PER_BATCH, SEQ_LENGTH, N_BINS+1))
    for i_batch in range(N_SEQ_PER_BATCH):
        for i_element in range(SEQ_LENGTH):
            data = inp[i_batch, i_element, 0]
            hist, _ = np.histogram(abs(data), bins=N_BINS, range=(0, 1))
            where = np.where(hist==1)[0][0]
            hist[:where] = 1
            out[i_batch,i_element,1:] = hist
            out[i_batch,i_element,0] = data < 0
    return (out * 2) - 1
