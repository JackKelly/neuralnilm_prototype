...
def exp_h(name):
    ...
    net_dict_copy['layers_config'] = [
        ...
        },
        {
            ...
            'nonlinearity': None,
            'border_mode': 'full',
            'W': 'ref:conv1.W.dimshuffle(1, 0, 2)[:, :, ::-1]',
            'shared_weights': True
        },
        {
            'type': DeConv1DLayer,
...
def exp_i(name):
    # based on 445q
    # separate biases
    # two conv layers (untied weights)
    global source
    source_dict_copy = deepcopy(source_dict)
    source = RealApplianceSource(**source_dict_copy)
    net_dict_copy = deepcopy(net_dict)
...
def exp_j(name):
    # like g but with untied weights for just the conv layers
    #  separate biases
    #  based on s
    # like v but with a Dense linear layer
    global source
...
def exp_k(name):
    # like g but with untied weights all through
    #  separate biases
    #  based on s
    # like v but with a Dense linear layer
    global source
...
def exp_l(name):
    # like g but with untied weights for dense layers, tied for convs
    #  separate biases
    #  based on s
    # like v but with a Dense linear layer
    global source
...
Emacs variables
Local Variables:
compile-command: "cp /home/jack/workspace/python/neuralnilm/scripts/e446.py /mnt/sshfs/imperial/workspace/python/neuralnilm/scripts/"
End:
"""
